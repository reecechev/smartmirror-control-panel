...
def get_public_url(max_wait=30):
    ...
    Poll the local ngrok API until an HTTPS public_url is present,

    or raise after max_wait seconds.
...
def post_render(base, max_wait=120):
    ...
    Wake Render by hitting '/', then POST to /ngrok with backoff retries.
...
def main():
    ...
    base = get_public_url(max_wait=30)   # <-- waits here until ngrok is ready
